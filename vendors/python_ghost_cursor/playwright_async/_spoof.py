...
class GhostCursor:
    ...
    async def captcha_click(
        ...
        self.toggle_random_move(False)
        if selector is not None:
            await self.captcha_move(selector, padding_percentage, wait_for_selector)
            self.toggle_random_move(False)
    ...
    async def captcha_move(
        ...
        captcha_box = {
            "x": box["x"] + 16,
            "y": box["y"] + 21,
            "width": 160,  # Small click area
            "height": 24,  # Small click area
        }
        if box is None:
            raise Exception(
                "Could not find the dimensions of the element you're clicking on, this might be a bug?"
            )
        destination = get_captcha_box_point(captcha_box, padding_percentage)
        dimensions = {"height": captcha_box["height"], "width": captcha_box["width"]}
        overshooting = should_overshoot(self.previous, destination)
        to = (
            overshoot(destination, self.overshoot_radius)
            if overshooting
